...
- ノードはフリーフォームノードとして作成し、接続は明示エッジとして作成する。
- インポート時の自動配置は `flowchart LR/TB/BT/RL` を反映する。連結成分ごとに分離し、ランク内の並びは接続先/接続元の重心で整える。
- subgraph は、配置済みノードの外接矩形を囲むセクションとして作成する。
- 未対応行は無視し、インポート後に件数をtoastで通知する。

<!-- 2026-06-15 / #19: Mermaid import を追加。まずはMindCanvasのMermaid exportで出す基本形式を戻せることを優先する。 -->
<!-- 2026-06-17 / #28: Mermaid import の自動配置を、方向・連結成分・重心並びを考慮するレイアウトへ改善。 -->
